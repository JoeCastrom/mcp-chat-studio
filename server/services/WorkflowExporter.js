...
`;
    ...
    for (const serverName of usedServers) {
      ...
      if (config.type === 'stdio' || config.command) {
        ...
        print(f"✅ Connected to ${serverName}")
`;
      } else {
...
`;
        ...
        code += `
  {
    ...
    await client.connect(transport);
    servers["${serverName}"] = client;
    console.log("✅ Connected to ${serverName} (SSE)");
  }
`;
      }
    }
...
`;
      if (node.type === 'trigger') {
        ...
        const data = node.data || {};
        code += `  try {
    console.log("▶️ Executing ${node.id}...");
    ...
    console.log("  ✅ Result:", output?.slice ? output.slice(0, 80) : output);
  } catch (err) {
...
`;
        ...
        code += `  try {
    console.log("▶️ Executing ${node.id} (LLM)...");
