...
function switchTab(tabName) {
  if (document.body.classList.contains('workspace-mode')) {
    if (
      ...
      typeof floatingWorkspace.togglePanel === 'function'
    ) {
...
async function loadServerResources() {
  ...
  if (!serverName) {
    listEl.innerHTML =
      '<div style="color: var(--text-muted); font-style: italic; padding: 12px;">Select a server to view available resources...</div>';
    document.getElementById('resourceContentSection').style.display = 'none';
    return;
  }

  listEl.innerHTML =
    '<div style="color: var(--text-muted); padding: 12px;">Loading resources...</div>';

  try {
    const response = await fetch(`/api/mcp/resources/${encodeURIComponent(serverName)}`, {
      credentials: 'include',
    });
    ...
    if (resources.length === 0) {
      listEl.innerHTML =
        '<div style="color: var(--text-muted); padding: 12px;">No resources available from this server.</div>';
      return;
    }
...
async function readResource(serverName, uri) {
  ...
  try {
    ...
    const content =
      data.contents?.[0]?.text || data.contents?.[0]?.blob || JSON.stringify(data, null, 2);
...
async function loadServerPrompts() {
  ...
  if (!serverName) {
    listEl.innerHTML =
      '<div style="color: var(--text-muted); font-style: italic; padding: 12px;">Select a server to view available prompts...</div>';
    document.getElementById('promptPreviewSection').style.display = 'none';
    return;
  }

  listEl.innerHTML =
    '<div style="color: var(--text-muted); padding: 12px;">Loading prompts...</div>';

  try {
    const response = await fetch(`/api/mcp/prompts/${encodeURIComponent(serverName)}`, {
      credentials: 'include',
    });
    const data = await response.json();
    ...
    if (prompts.length === 0) {
      listEl.innerHTML =
        '<div style="color: var(--text-muted); padding: 12px;">No prompts available from this server.</div>';
      return;
    }
...
function loadVariableStore() {
  try {
    ...
    return {
      global:
        data && typeof data.global === 'object' && !Array.isArray(data.global) ? data.global : {},
      environments:
        data && typeof data.environments === 'object' && !Array.isArray(data.environments)
          ? data.environments
          : {},
    };
  } catch (e) {
...
function toggleInspectorAuth(enabled, skipSave) {
  ...
  updateInspectorAuthStatus(
    enabled ? 'Auth overrides enabled for this request.' : 'Auth overrides disabled.'
  );
...
function buildInspectorAuthOverrides(tool, variables, silent = false) {
  ...
  if (!toolSupportsInspectorAuth(tool)) {
    const message =
      'Auth overrides ignored. This tool does not support OpenAPI-style auth overrides.';
    if (!silent) {
...
async function showVariablesManager() {
  ...
  const result = await appFormModal({
    ...
    message:
      'Global variables apply everywhere. Environment variables override globals. Inspector variables override both when running tools.',
...
function generateInspectorForm() {
  ...
  for (const [paramName, paramDef] of Object.entries(schema.properties)) {
    ...
    if (paramDef.type === 'boolean') {
      ...
      const placeholder = paramDef.type === 'array' ? '["item1", "item2"]' : '{"key": "value"}';
...
async function executeInspectorTool() {
  ...
  try {
    ...
    // Log the response
    logProtocolMessage('response', selectedInspectorTool.name, data.error || data.result, duration);
...
async function runInspectorMatrix() {
  ...
  if (baselineSelect && lastInspectorRequest.serverName) {
    const hasBaseline = Array.from(baselineSelect.options || []).some(
      option => option.value === lastInspectorRequest.serverName
    );
    if (hasBaseline) {
...
function copyInspectorResponse() {
  ...
  const json = JSON.stringify(lastInspectorResponse.error || lastInspectorResponse.result, null, 2);
