...
const assistantFAQ = [
  {
    ...
    answer:
      'Use the **MCP Servers** sidebar → **Add** → pick a template or fill in command/args. Click **Connect** to see tools.',
  },
  {
    title: 'Run a tool',
    keywords: ['inspector', 'run tool', 'execute', 'tool call'],
    answer:
      'Go to **Inspector**, select server + tool, fill args, then **Execute**. Results show below with protocol logs.',
  },
  {
    title: 'Record scenarios',
    keywords: ['scenario', 'record', 'replay', 'recording'],
    answer:
      'Open **Scenarios**, click **Start Recording**, then execute tools in Inspector. Stop and **Save Scenario**.',
  },
  {
    title: 'Collections + reports',
    keywords: ['collection', 'run report', 'runner', 'iterations'],
    answer:
      'Create a **Collection**, add scenarios, then **Run**. The report shows pass/fail + exports (JSON/JUnit).',
  },
  {
    title: 'Matrix runs',
    keywords: ['matrix', 'compare servers', 'cross-server'],
    answer:
      'Use **Scenario → Matrix** or **Inspector → Diff → Cross‑Server Snapshot**. Connect 2+ servers for heatmaps.',
  },
  {
    title: 'Generator (OpenAPI)',
    keywords: ['generator', 'openapi', 'proxy', 'generate server'],
    answer:
      'Open **Generator**, load an OpenAPI spec, select endpoints, then **Generate Code** or **Download ZIP**. OpenAPI Proxy mode creates a real MCP server that calls your API.',
  },
  {
    ...
    answer:
      'Click **Test in Studio** after generating. Paste the folder path that contains `server.py` (Python) or `server.js` (Node), plus `requirements.txt` or `package.json`. Studio cannot detect paths automatically.',
  },
  {
    title: 'Assistant OpenAPI import',
    keywords: ['assistant', 'upload', 'paste', 'openapi url', 'openapi json'],
    answer:
      'Paste an OpenAPI URL/JSON here or click the 📎 button to upload a spec. I can import it into the Generator for you.',
  },
  {
    title: 'Quick Start',
    keywords: ['quick start', 'start', 'getting started', 'guide'],
    answer:
      'Use the **🚀 Start** button in the header for a guided checklist with jump‑buttons to Inspector, Scenarios, Collections, Contracts, and Generator.',
  },
  {
    title: 'Contracts',
    keywords: ['contract', 'schema', 'breaking change', 'regression'],
    answer:
      'Go to **Contracts** → **New Contract**. Validate responses and use **Schema Regression** to diff snapshots.',
  },
  {
    title: 'Mocks',
    keywords: ['mock', 'mock server', 'offline'],
    answer:
      'Go to **Mocks**, create or **From History**, then **Connect** to register as an MCP server.',
  },
  {
    title: 'Workflow debugger',
    keywords: ['debugger', 'workflow', 'breakpoint', 'step'],
    answer:
      'Create a workflow, then open **Debugger** to start a session, step nodes, and inspect variables.',
  },
  {
    title: 'Workspace mode',
    keywords: ['workspace', 'panels', 'layout', 'dock'],
    answer:
      'Toggle **Workspace** in the header. Right‑click to add panels, use the mini‑map + zoom, and save sessions.',
  },
  {
    title: 'LLM settings',
    keywords: ['llm', 'provider', 'model', 'apikey', 'api key'],
    answer:
      'Click the **⚙️** button to set provider/model/base URL, add API keys, or pick visible providers.',
  },
];
...
function applyAssistantLayout() {
  ...
  const hasCustom =
    assistantCustomSize &&
    Number.isFinite(assistantCustomSize.width) &&
    Number.isFinite(assistantCustomSize.height);
...
document.addEventListener('mousemove', event => {
  ...
  const height = Math.max(
    minHeight,
    Math.min(maxHeight, assistantResizeState.startHeight + deltaY)
  );
...
function updateAssistantContextLabel() {
  ...
  label.textContent = `Context: ${ctx.layout} ${suffix} ${lastTool} ${status}`
    .replace(/\s+/g, ' ')
    .trim();
}
...
function renderAssistantMessages() {
  ...
  if (!assistantMessages.length) {
    list.innerHTML =
      '<div class="assistant-message assistant">Ask me anything about MCP Chat Studio.</div>';
    return;
  }
...
function renderAssistantChips() {
  ...
  chipWrap.innerHTML = assistantRecentCommands
    .map(cmd => {
...
async function tryMainChatOpenApi(content) {
  ...
  if (url) {
    ...
    if (document.body.classList.contains('workspace-mode')) {
      if (
        typeof floatingWorkspace !== 'undefined' &&
        floatingWorkspace.panels?.some(p => p.type === 'generator')
      ) {
...
async function tryAssistantOpenApi(content) {
  ...
  if (jsonSpec) {
    ...
    assistantMessages.push({
      role: 'assistant',
      content: '✅ OpenAPI JSON imported into Generator.',
    });
    assistantMessages.push({
      type: 'action',
      content: 'Next: generate and test the MCP proxy.',
...
function openPanelByName(panelKey) {
  ...
  if (document.body.classList.contains('workspace-mode')) {
    if (
      typeof floatingWorkspace !== 'undefined' &&
      typeof floatingWorkspace.focusPanelByType === 'function'
    ) {
...
async function tryAssistantAction(message) {
  ...
  const panelKeyFromText = Object.keys(panelKeywords).find(key =>
  ...
  if (hasVerb(openVerbs)) {
    if (panelKeyFromText) {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Opened ${panelKeywords[panelKeyFromText][0].replace(/^\w/, c => c.toUpperCase())}.`,
      });
      return true;
    }
  }
  ...
  if (
    ...
    assistantMessages.push({
      role: 'assistant',
      content:
        'Drop an OpenAPI JSON/YAML file onto this panel or click the **📎** button. Browsers block auto‑opening the file picker from text commands.',
    });
    return true;
  }
  ...
  if (
    text.includes('build workspace') ||
    text.includes('new workspace') ||
    text.includes('reset workspace')
  ) {
    if (!document.body.classList.contains('workspace-mode')) {
      setLayoutMode('workspace');
    }
    ...
    const confirmed = await appConfirm(
      'This will replace your current workspace layout. Continue?',
      {
    ...
    if (preset && typeof floatingWorkspace !== 'undefined') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Loaded workspace preset "${preset}".`,
      });
      return true;
    }
    ...
    if (requestedPanels.length > 0 && typeof floatingWorkspace !== 'undefined') {
      ...
      requestedPanels.forEach((type, index) => {
        floatingWorkspace.addPanel(
          type,
          100 + (index % 3) * 400,
          80 + Math.floor(index / 3) * 300,
          {
            skipSave: true,
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Built workspace with ${requestedPanels.join(', ')}.`,
      });
      return true;
    }
    ...
    if (typeof floatingWorkspace !== 'undefined') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: '✅ Loaded workspace preset "development".',
      });
      return true;
    }
  }
  ...
  if (text.includes('toggle grid') || text.includes('grid snap')) {
    if (typeof floatingWorkspace !== 'undefined') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Grid snap ${floatingWorkspace.snapToGrid ? 'enabled' : 'disabled'}.`,
      });
      return true;
    }
  }
  ...
  if (text.includes('add panel') && panelKeyFromText) {
    if (openWorkspacePanel(panelKeyFromText)) {
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Added ${panelKeywords[panelKeyFromText][0]} panel.`,
      });
      return true;
    }
  }
  ...
  if ((text.includes('close panel') || text.includes('remove panel')) && panelKeyFromText) {
    if (closeWorkspacePanel(panelKeyFromText)) {
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Closed ${panelKeywords[panelKeyFromText][0]} panel.`,
      });
      return true;
    }
  }
  ...
  if (text.includes('resize') && panelKeyFromText) {
    ...
    if (!sizeMatch) {
      assistantMessages.push({
        role: 'assistant',
        content: 'Please provide a size, e.g., "resize inspector 600x500".',
      });
      return true;
    }
    ...
    const confirmed = await appConfirm(
      `Resize ${panelKeywords[panelKeyFromText][0]} to ${width}x${height}?`,
      {
        title: 'Resize Panel',
        confirmText: 'Resize',
      }
    ...
    if (resizeWorkspacePanel(panelKeyFromText, width, height)) {
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Resized ${panelKeywords[panelKeyFromText][0]} to ${width}x${height}.`,
      });
      return true;
    }
  }
  ...
  if ((text.includes('minimize') || text.includes('collapse')) && panelKeyFromText) {
    if (minimizeWorkspacePanel(panelKeyFromText, true)) {
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Minimized ${panelKeywords[panelKeyFromText][0]} panel.`,
      });
      return true;
    }
  }
  ...
  if ((text.includes('maximize') || text.includes('restore')) && panelKeyFromText) {
    if (minimizeWorkspacePanel(panelKeyFromText, false)) {
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Restored ${panelKeywords[panelKeyFromText][0]} panel.`,
      });
      return true;
    }
  }
  ...
  if (text.includes('save workspace') || text.includes('save session')) {
    if (typeof floatingWorkspace !== 'undefined') {
      const name =
        extractName(['save workspace', 'save session']) ||
        (await appPrompt('Session name:', {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Saved workspace session "${name}".`,
      });
      return true;
    }
  }
  ...
  if (text.includes('load session') || text.includes('load workspace')) {
    if (typeof floatingWorkspace !== 'undefined') {
      ...
      if (!sessions.length) {
        assistantMessages.push({
          role: 'assistant',
          content: 'No saved workspace sessions found.',
        });
        return true;
      }
      if (!name) {
      ...
      if (matches.length !== 1) {
        assistantMessages.push({
          ...
          content: matches.length
            ? `Multiple sessions match: ${matches.map(m => `"${m.name}"`).join(', ')}`
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Loaded workspace session "${matches[0].name}".`,
      });
      return true;
    }
  }
  ...
  if (text.includes('delete session') || text.includes('remove session')) {
    if (typeof floatingWorkspace !== 'undefined') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Deleted workspace session "${matches[0].name}".`,
      });
      return true;
    }
  }
  ...
  if (text.includes('import workspace')) {
    if (typeof floatingWorkspace !== 'undefined') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: '✅ Choose a workspace bundle to import.',
      });
      return true;
    }
  }
  ...
  if (hasVerb(runVerbs) && (text.includes('matrix') || text.includes('cross-server'))) {
    if (typeof runInspectorMatrix === 'function') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: '✅ Running cross‑server matrix from the last Inspector request.',
      });
      return true;
    }
  }
  ...
  if (hasVerb(runVerbs) && text.includes('scenario')) {
    ...
    if (!scenarios.length) {
      assistantMessages.push({
        role: 'assistant',
        content: 'No scenarios saved yet. Record one from Inspector → Scenarios.',
      });
      return true;
    }
    ...
    if (scenarioName) {
      ...
      if (matches.length === 1) {
        ...
        assistantMessages.push({
          role: 'assistant',
          content: `I couldn't find a scenario named "${scenarioName}". Try another name.`,
        });
        return true;
      }
    }
  ...
  if (hasVerb(runVerbs) && text.includes('collection')) {
    ...
    try {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `Could not load collections: ${error.message}`,
      });
      return true;
    }
    if (!collections.length) {
      assistantMessages.push({
        role: 'assistant',
        content: 'No collections found. Create one in the Collections tab.',
      });
      return true;
    }
    ...
    if (collectionName) {
      ...
      if (matches.length === 1) {
        ...
        assistantMessages.push({
          role: 'assistant',
          content: `I couldn't find a collection named "${collectionName}". Try another name.`,
        });
        return true;
      }
    }
    ...
    if (typeof runCollection === 'function') {
      ...
      assistantMessages.push({
        role: 'assistant',
        content: `✅ Running collection "${target.name}".`,
      });
    } else {
      assistantMessages.push({
        role: 'assistant',
        content: 'Collection runner is not available in this view.',
      });
    }
    return true;
  }
  ...
  if (text.includes('start recording')) {
    ...
    assistantMessages.push({
      role: 'assistant',
      content: '✅ Recording started. Execute tools in Inspector.',
    });
    return true;
  }
...
async function updateModelBadge() {
  try {
    ...
    document.getElementById('modelProvider').textContent = providerEmojis[config.provider] || '🤖';
