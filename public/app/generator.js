...
function renderOpenApiTagChips() {
  ...
  wrap.innerHTML = Array.from(tagCounts.entries())
    .map(([tag, count]) => {
...
function formatOpenApiAuthSummary(auth) {
  ...
  const parts = auth.schemes.map(scheme => {
    ...
    if (scheme.type === 'oauth2') {
      const flow =
        Array.isArray(scheme.flows) && scheme.flows.length ? `:${scheme.flows.join(',')}` : '';
      return `oauth2${flow}`;
    }
    if (scheme.type === 'openIdConnect') {
      return 'oidc';
...
function renderOpenApiEndpoints() {
  ...
  if (!openApiOperations.length) {
    list.innerHTML =
      '<div style="color: var(--text-muted); font-style: italic; text-align: center; padding: 12px;">Load a spec to see endpoints.</div>';
    return;
  }
  const ops = getFilteredOpenApiOps();
  if (!ops.length) {
    list.innerHTML =
      '<div style="color: var(--text-muted); font-style: italic; text-align: center; padding: 12px;">No endpoints match your filters.</div>';
    return;
  }
  list.innerHTML = ops
    .map(op => {
      ...
      const sourceTag =
        op.source === 'webhook'
          ? '<span style="font-size: 0.55rem; padding: 2px 6px; border-radius: 999px; background: rgba(14, 165, 233, 0.2); border: 1px solid rgba(14, 165, 233, 0.5); color: #7dd3fc;">Webhook</span>'
          : '';
      return `
...
function updateOpenApiSummary() {
  ...
  const endpointNote = openApiOperations.length
    ? `${openApiOperations.length} endpoints`
    : 'No endpoints found';
...
function renderGeneratorTools() {
  ...
  listEl.innerHTML = generatorTools
    .map(
      (tool, idx) => `
...
function generateMCPCode() {
  ...
  appendMessage(
    'system',
    `🚀 Generated ${language} MCP server code (${mode}) with ${generatorTools.length} tools`
  );
}
...
function getGeneratorRunCommands(os, language) {
  ...
  return ['npm install', 'node server.js'].join('\\n');
}
...
async function saveGeneratorToFolder() {
  ...
  try {
    ...
    const projectHandle = await rootHandle.getDirectoryHandle(folderName || 'mcp-server', {
      create: true,
    });
...
function updateGeneratorTestModalConfig(initial) {
  const serverName =
    initial?.serverName || document.getElementById('genServerName').value || 'my-mcp-server';
  const language = document.getElementById('genLanguage').value;
  const baseUrl =
    initial?.baseUrl || document.getElementById('genServerBaseUrl')?.value?.trim() || '';
  ...
  const yamlLines = [
    ...
    `    command: ${cmd}`,
    ...(generatorTestCwd
      ? [`    cwd: ${generatorTestCwd}`]
      : ['    cwd: /path/to/generated/project']),
    '    args:',
    ...args.map(arg => `      - ${arg}`),
  ];
  if (baseUrl) {
...
function buildGeneratorServerPayload(overrideName) {
  const name =
    overrideName || document.getElementById('genServerName').value.trim() || 'my-mcp-server';
  const description =
    document.getElementById('genServerDesc').value.trim() || `Generated MCP server: ${name}`;
  const language = document.getElementById('genLanguage').value;
  const baseUrl = document.getElementById('genServerBaseUrl')?.value?.trim() || '';
...
async function addGeneratorServerToStudio({ connectAfter = false, overrideName = null } = {}) {
  ...
  try {
    ...
    const endpoint = serverExists
      ? `/api/mcp/update/${encodeURIComponent(payload.name)}`
      : '/api/mcp/add';
...
async function ensureGeneratorToolsForRun() {
  ...
  if (openApiSelected && openApiSelected.size > 0) {
    const confirmed = await appConfirm(
      'No tools yet. Import the selected OpenAPI endpoints and continue?',
      {
...
async function runGeneratorAndConnect() {
  ...
  if (serverName !== desiredName) {
    notifyUser(
      `Using temporary server name "${serverName}" to avoid overwriting "${desiredName}".`,
      'info'
    );
  }
  ...
  if (language === 'python') {
    const confirmed = await appConfirm(
      'Auto-run uses your local Python and requires the mcp package installed. Continue?',
      {
        title: 'Run & Connect (Auto)',
  ...
  if (mode === 'openapi' && !baseUrl) {
    const confirmed = await appConfirm(
      'No API Base URL set. Auto-run will start but tool calls may fail until you set API_BASE_URL. Continue?',
      {
        title: 'Run & Connect (Auto)',
        confirmText: 'Continue',
      }
  ...
  try {
    ...
    const connected = await addGeneratorServerToStudio({
      connectAfter: true,
